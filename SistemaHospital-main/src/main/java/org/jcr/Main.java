...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        InMemoryRepository<Paciente> pacienteRepo = new InMemoryRepository<>();
        InMemoryRepository<Medico> medicoRepo = new InMemoryRepository<>();
        try {
            ...
            // 4. Guardar medicos y pacientes en la memoria
            for (Paciente p : pacientes) pacienteRepo.save(p);
            for (Medico m : medicos) medicoRepo.save(m);
            ...
            // 10. Utilizar InMemoryRepository
            System.out.println("\n===== Probando Reflection... =====");
            pacienteRepo.findById(1L).ifPresent(p -> System.out.println("\nPaciente con id=1: " + p.getNombreCompleto()));
            List<Paciente> resultados = pacienteRepo.genericFindByField("dni", "22222222");
            System.out.println("Pacientes con DNI buscado " + resultados);
    ...
    private static Hospital inicializarHospital() {
        ...
        // Crear hospital principal usando builder
        ...
        // Crear departamentos especializados usando builders
        ...
        // Crear salas por departamento (mantenemos el metodo que crea la sala y la asocia)
    ...
    private static void crearSalasPorDepartamento(Departamento cardiologia, Departamento pediatria, Departamento traumatologia) {
        ...
        cardiologia.crearSala("CARD-102", "Quirofano");
    ...
    private static List<Medico> crearMedicos(Hospital hospital) {
        System.out.println("Registrando medicos especialistas...");

        List<Medico> medicos = new ArrayList<>();

        // Crear médicos especialistas usando builders (el builder llama al constructor validante)
        ...
        System.out.println("Registrados " + medicos.size() + " medicos especialistas\n");
        return medicos;
    }
    ...
    private static List<Paciente> registrarPacientes(Hospital hospital) {
        ...
        // Crear pacientes con diferentes perfiles usando builders
        ...
        System.out.println("Registrados " + pacientes.size() + " pacientes con historias clinicas\n");
        return pacientes;
    }
    ...
    private static void configurarHistoriasClinicas(Paciente pacienteCardiaco, Paciente pacientePediatrico, Paciente pacienteTraumatologico) {
        ...
        pacienteCardiaco.getHistoriaClinica().agregarDiagnostico("Hipertension arterial");
        ...
        pacientePediatrico.getHistoriaClinica().agregarDiagnostico("Control pediatrico rutinario");
        pacientePediatrico.getHistoriaClinica().agregarTratamiento("Vacunas al dia");

        // Historia clínica del paciente traumatológico
        pacienteTraumatologico.getHistoriaClinica().agregarDiagnostico("Fractura de brazo");
        pacienteTraumatologico.getHistoriaClinica().agregarTratamiento("Inmovilizacion y fisioterapia");
    ...
    private static void programarCitas(CitaManager citaManager, List<Medico> medicos, List<Paciente> pacientes, Hospital hospital) throws CitaException {
        System.out.println("Programando citas medicas...");
        ...
        citaCardiologica.setObservaciones("Paciente con antecedentes de hipertension");
        ...
        System.out.println("Programadas 3 citas medicas exitosamente\n");
    }
    ...
    private static void mostrarDepartamentosYPersonal(Hospital hospital) {
        ...
        for (Departamento dep : hospital.getDepartamentos()) {
            ...
            System.out.println("  Medicos (" + dep.getMedicos().size() + "):");
    ...
    private static void mostrarPacientesEHistorias(Hospital hospital) {
        ...
        for (Paciente paciente : hospital.getPacientes()) {
            ...
            System.out.println("  Historia: " + historia.getNumeroHistoria() + " | Edad: " + paciente.getEdad());

            if (!historia.getDiagnosticos().isEmpty()) {
                System.out.println("  Diagnosticos: " + historia.getDiagnosticos());
            }
    ...
    private static void probarValidacionFechaPasado(CitaManager citaManager, Paciente paciente, Medico medico, Sala sala) {
        try {
            ...
            System.out.println("✗ ERROR: Se permitio cita en el pasado");
        } catch (CitaException e) {
            System.out.println("✓ Validacion fecha pasado: " + e.getMessage());
        }
    }
    ...
    private static void probarValidacionCostoNegativo(CitaManager citaManager, Paciente paciente, Medico medico, Sala sala) {
        try {
            ...
            System.out.println("✗ ERROR: Se permitio costo negativo");
        } catch (CitaException e) {
            System.out.println("✓ Validacion costo negativo: " + e.getMessage());
        }
    }
    ...
    private static void probarValidacionEspecialidadIncompatible(CitaManager citaManager, Paciente paciente, List<Medico> medicos, Hospital hospital) {
        try {
            ...
            citaManager.programarCita(paciente, cardiologo, salaPediatria,
                    ...
                    new BigDecimal("100000.00"));
            System.out.println("✗ ERROR: Se permitio especialidad incompatible");
        } catch (CitaException e) {
            System.out.println("✓ Validacion especialidad incompatible: " + e.getMessage());
        }
    }
    ...
    private static void mostrarEstadisticasFinales(Hospital hospital) {
        ...
        System.out.println("Medicos: " + totalMedicos);
    ...
    private static void mostrarDistribucionTipoSangre(Hospital hospital) {
        System.out.println("\nDistribucion por tipo de sangre:");
